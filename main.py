...
class App:
    def __init__(self, master):
        ...
        filemenu.add_command(label='Refresh List', command=self.update_movie_list)
        ...
        movie_content.grid(sticky='NSEW')
        movie_content['padx'] = 10
        movie_content['pady'] = 10
        for x in range(100):
            movie_content.rowconfigure(x, weight=1)

        # List of movies
        file_list_frame = tk.Frame(movie_content)
        file_list_frame.grid(row=1, column=0, sticky='NSEW', rowspan=100)
        ...
        # Title
        title_frame = tk.LabelFrame(movie_content, text='Title')
        title_frame.grid(column=3, row=1, sticky='NW')
        ...
        # Rating
        rating_frame = tk.LabelFrame(movie_content, text='Rating')
        rating_frame.grid(column=3, row=2, sticky='NW')

        self.rating_string = tk.StringVar()
        self.rating_string.set('No selections')
        rating_text = tk.Label(rating_frame, textvariable=self.rating_string, wraplength=400, justify=tk.LEFT)
        rating_text.grid()

        # Release Date
        release_date_frame = tk.LabelFrame(movie_content, text='Release Date')
        release_date_frame.grid(column=3, row=3, sticky='NW')
        ...
        # Open File
        open_file_button = ttk.Button(movie_content, text='Open Movie', command=self.run_movie)
        open_file_button.grid(column=3, row=5, sticky='NW')

        # Description
        description_frame = tk.LabelFrame(movie_content, text='Description')
        description_frame.grid(column=3, row=6, sticky='NW')
        ...
        statusbar.grid(column=0, row=1000, rowspan=1000, columnspan=1000, sticky='SEW')
        # master.rowconfigure(1000, weight=1)
        master.columnconfigure(0, weight=1)

        # Delete File
        delete_file_button = ttk.Button(movie_content, text='Remove From List', command=self.delete_movie)
    ...
    def list_select(self, event):
        ...
        self.description_string.set(self.Db.get_description(title))
